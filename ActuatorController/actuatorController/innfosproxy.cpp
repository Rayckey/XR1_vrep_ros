...
void InnfosProxy::decode(quint32 communicateUnitId, QByteArray &buf)
{
    ...
    case D_READ_CUR_CURRENT:
    {
        ...
        qint32 nValue = data.ReadInt();
        qreal real = nValue;
        real = (real)*motorDataMgrInstance->getMotorDataAttrValueAsDouble(nDeviceId,CURRENT_SCALE) / (1 << 24);
        Mediator::getInstance()->SetCurParam(nDeviceId,real,nMode);
    ...
    case D_READ_CURRENT_MAXSPEED:
    {
        ...
        qint32 nValue = data.ReadInt();
        qreal real = nValue;
        real = (real)*motorDataMgrInstance->getMotorDataAttrValueAsDouble(nDeviceId,VELOCITY_SCALE) / (1 << 24);
        Mediator::getInstance()->SetCurParam(nDeviceId,real,nMode);
    ...
    case D_READ_CHANNEL_1:
    {
        ...
        Mediator::getInstance()->chartVauleChange(Actuator::channel_1,real);
    }
        break;
    case D_READ_CHANNEL_2:
    {
        ...
        qreal real = nValue;
        real = (real)/**curScale*/ / (1 << 15);
        Mediator::getInstance()->chartVauleChange(Actuator::channel_2,real);
    }
        break;
    case D_READ_CHANNEL_3:
    {
        ...
        Mediator::getInstance()->chartVauleChange(Actuator::channel_3,real);
    }
        break;
    case D_READ_CHANNEL_4:
    {
        ...
        qreal real = nValue;
        real = (real) / (1 << 15);
        Mediator::getInstance()->chartVauleChange(Actuator::channel_4,/*real*16*/real);
    }
        break;
...
//         quint32 nMac = 1000;
    ...
    case D_READ_MOTORS_SWITCH:
    {
        ...
        break;
    case D_READ_CUMULATIVE_TIME:
    {
        data.Skip(5);
        quint32 nTime = data.ReadUInt();
        Mediator::getInstance()->SetCurParam(nDeviceId,nTime,nMode);
    }
        break;
    case D_CRC_ERROR:
    {
...
void InnfosProxy::SendQrealProxy(const quint8 nDeviceId, const int nIdx, qreal data)
{
    ...
    {
        int nScaleValue = data/motorDataMgrInstance->getMotorDataAttrValueAsDouble(nDeviceId,CURRENT_SCALE)*(1<<24);
        SendProxy(nDeviceId,nIdx,nScaleValue);
    }
        break;
    case D_SET_VELOCITY:
    ...
    {
        int nScaleValue = data/motorDataMgrInstance->getMotorDataAttrValueAsDouble(nDeviceId,VELOCITY_SCALE)*(1<<24);
        SendProxy(nDeviceId,nIdx,nScaleValue);
    }
        break;
...
void InnfosProxy::decodeTmpCmd(quint32 communicateUnitId, QByteArray &buf)
{
    ...
    case T_D_READ_QUATERNION:
    {
        ...
        break;
    case T_D_READ_ALL_QUATERNION:
    {
        if(nDataLen != 305)
        {
            qDebug() << "Data lenth error:" << nDataLen;
            return;
        }
        //quint8 nPackageCnt = (nDataLen-1)/17;
        for(int i=0;i<7;++i)//手掌以外关节
        {
            //quint8 nIMUId = data.ReadUByte();
            double w = data.ReadInt()*1.0/(1<<30);
            double x = data.ReadInt()*1.0/(1<<30);
            double y = data.ReadInt()*1.0/(1<<30);
            double z = data.ReadInt()*1.0/(1<<30);
            mediator->receiveQuaternion(i,w,x,y,z);
        }

        for(int i=0;i<2;++i)//手掌关节
        {
            for(int j=0;j<6;++j)
            {
                double w = data.ReadInt()*1.0/(1<<30);
                ...
                double y = data.ReadInt()*1.0/(1<<30);
                double z = data.ReadInt()*1.0/(1<<30);
                mediator->receiveQuaternion(i+2*j+7,w,x,y,z);
            }
        }
    }
        break;
...
void NoCrcProxy::decode(quint32 communicateUnitId, QByteArray &buf)
{
    ...
    case D_SAVE_PARAM:
        ...
        {
            Mediator::getInstance()->SetFailed(nDeviceId,nMode);
        }
        break;
...
//        quint8 nSwitch = motorDataMgrInstance->getMotorDataAttrValueAsInt(nDeviceId,Actuator::ACTUATOR_SWITCH);
//        if(nValue == 0)//如果设置失败了，保持原来的值不变
//        {
//            Mediator::getInstance()->SetCurParam(nDeviceId,nSwitch==Actuator::MOTOR_SWITCH_ON?Actuator::ACTUATOR_SWITCH_OFF:Actuator::MOTOR_SWITCH_ON,nMode);
//        }
//        else
//        {
...
//        break;
    ...
    case D_READ_LAST_STATE:
        if (buf.at(5) == '\x0')
        {
            mediator->m_sError.s_Emit(nDeviceId,Actuator::ERR_ABNORMAL_SHUTDOWN,tr("Last abnormal shutdown of the device ID %1, please check \n the current location of each device is normal!").arg(nDeviceId).toStdString());
