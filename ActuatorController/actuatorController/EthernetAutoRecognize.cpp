    ...
    m_pSocket(nullptr)
{
    ...
    QString localHost = "192.168.1.111";
    //QHostAddress addr = QHostAddress(localHost);
    QHostAddress addr = QHostAddress::AnyIPv4;
    if(m_pSocket->bind(QHostAddress(localHost),2001,QAbstractSocket::ShareAddress))
    {
...
{
    qint64 nLen = m_pSocket->writeDatagram(InnfosProxy::getProxyContent(0,Actuator::D_CAN_CONNECT),QHostAddress("192.168.1.255"),2000);
    qDebug() << "write" << nLen << InnfosProxy::getProxyContent(0,Actuator::D_CAN_CONNECT).toHex();
    QTimer::singleShot(300,[=]{
        qDebug() << "udp recieve" << m_pSocket->hasPendingDatagrams();
        onIpBroadcast();
        if(!m_bFindAvaliable)
        {
            //mediator->errorOccur(0,Actuator::ERR_IP_ADDRESS_NOT_FOUND,"No available ip address!");
            waitTimeout();
            qDebug() << "failed";
        }
        else
        {
            ...
            foreach (QHostAddress addr, m_addrVector)
            {
                ...
                mediator->setUnitConnectionStatus(nId,Actuator::CAN_CONNECTED);
                //InnfosProxy::SendProxy(0,D_CAN_CONNECT);               
                ++i;
            }
            InnfosProxy::SendProxy(0,Actuator::D_READ_ADDRESS);
...
void EthernetAutoRecognize::onIpBroadcast()
{
    ...
    while (m_pSocket->hasPendingDatagrams()) {
        ...
        if((quint8)datagram.at(0)==0xee && (quint8)datagram.at(2) == Actuator::D_CAN_CONNECT)
        {
