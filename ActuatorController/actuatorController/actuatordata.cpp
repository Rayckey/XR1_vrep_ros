...
ActuatorData::ActuatorData(const quint8 nDeviceId, const quint32 nDeviceMac, QObject *parent) :
    QObject(parent),
    m_nHeartFailCnt(0),
    m_nAutoRequestInterval(1000)
{
    ...
    connect(m_pHeartTimer,&QTimer::timeout,[=]{
        if(!isOnline())
        {
            if(getValue(ONLINE_STATUS).toInt() != Actuator::Status_Offline)
            {
                setValue(ONLINE_STATUS,Actuator::Status_Offline);
            }
            //m_pHeartTimer->stop();
            qDebug() << "no stop";
            if(m_pValueTimer->isActive())
            {
                m_pValueTimer->stop();
            }
        }
    ...
    connect(m_pValueTimer,&QTimer::timeout,this,&ActuatorData::requestActualValue);
...
void ActuatorData::changeDemand(qreal value)
{
...
void ActuatorData::setAutoRequestInterval(quint32 mSec)
{
...
void ActuatorData::setHomingOperationMode(const quint8 nMode)
{
...
void ActuatorData::switchCalibrationVel(quint8 nValue)
{
    InnfosProxy::SendProxy(deviceId(),D_SWITCH_CALIBRATION_VEL,nValue);
}

void ActuatorData::switchCalibration(quint8 nValue)
{
...
void ActuatorData::requestSuccessfully(quint8 nDataId)
{
...
void ActuatorData::setValue(int nDataId, QVariant value, bool bEmitSignal)
{
    if(DATA_CNT > nDataId)
    {
        if(nDataId == DEVICE_ID)
        {
            if(bEmitSignal)//先发送信号，避免修改id以后无法通知关联旧id的对象
                motorDataMgrInstance->dataChanged(deviceId(),(ActuatorAttribute)nDataId,value);
...
void ActuatorData::userRequestValue(int nDataId, QVariant value)
{
    if(DATA_CNT > nDataId)
    {
...
{
    return (Actuator::ActuatorMode)(getValue(MODE_ID).toInt());
}

void ActuatorData::saveDataToFile(QString fileName)//todo
{
    QFile file(fileName);
...
void ActuatorData::readDataFromFile(QString fileName)
{
    QFile file(fileName);
...
quint8 ActuatorData::deviceId() const
{
//    if(m_oldDeviceId >=0)
...
qint8 ActuatorData::requestDeviceId() const
{
    return m_userRequestData[DEVICE_ID].toUInt();
}

quint32 ActuatorData::deviceMac() const
{
    return getValue(SN_ID).toUInt();
}

void ActuatorData::setValueByProxy(const int nProxyId, QVariant value)
{
    //responseHeart(true);
    if(!isOnline())
        responseHeart(true);
    ...
    case D_CHECK_ERROR:
    {
        ...
        errorInfo.insert(Actuator::ERR_ACTUATOR_OVERVOLTAGE,QString::fromLocal8Bit("ID:%1 voltage:%2 Overvoltage error!").arg(deviceId()).arg(getValue(VOLTAGE).toDouble()));
        errorInfo.insert(Actuator::ERR_ACTUATOR_UNDERVOLTAGE,QString::fromLocal8Bit("ID:%1 voltage:%2 Undervoltage error!").arg(deviceId()).arg(getValue(VOLTAGE).toDouble()));
        errorInfo.insert(Actuator::ERR_ACTUATOR_LOCKED_ROTOR,QString::fromLocal8Bit("ID:%1 Locked-rotor error!").arg(deviceId()));
        errorInfo.insert(Actuator::ERR_ACTUATOR_OVERHEATING,QString::fromLocal8Bit("ID:%1 Temperture:%2 Overheating error!").arg(deviceId()).arg(getValue(ACTUATOR_TEMPERATURE).toDouble()));
        errorInfo.insert(Actuator::ERR_ACTUATOR_READ_OR_WIRTE,QString::fromLocal8Bit("ID:%1 Read or write params error!").arg(deviceId()));
        errorInfo.insert(Actuator::ERR_ACTUATOR_MULTI_TURN,QString::fromLocal8Bit("ID:%1 Multi-turn count error!").arg(deviceId()));
        errorInfo.insert(Actuator::ERR_INVERTOR_TEMPERATURE_SENSOR,QString::fromLocal8Bit("ID:%1 Invertor temperature sensor error!").arg(deviceId()));
        errorInfo.insert(Actuator::ERR_ACTUATOR_TEMPERATURE_SENSOR,QString::fromLocal8Bit("ID:%1 Motor temperature sensor error!").arg(deviceId()));
        errorInfo.insert(Actuator::ERR_CAN_COMMUNICATION,QString::fromLocal8Bit("ID:%1 Can communication error!").arg(deviceId()));
        errorInfo.insert(Actuator::ERR_DRV_PROTECTION,QString::fromLocal8Bit("ID:%1 DRV protection error!").arg(deviceId()));
    ...
    case D_READ_MOTORS_SWITCH:
        if(value.toInt() == Actuator::ACTUATOR_SWITCH_ON && m_motorData[ACTUATOR_SWITCH].toInt()==Actuator::ACTUATOR_SWITCH_OFF)
        {
            requestAllValue();
        }
        setValue(ACTUATOR_SWITCH,value.toInt());
...
QVariant ActuatorData::getValue(Actuator::ActuatorAttribute nDataId) const
{
...
QVariant ActuatorData::getUserRequestValue(Actuator::ActuatorAttribute nDataId) const
{
    Q_ASSERT(nDataId < DATA_CNT);
    if(nDataId < DATA_CNT)
    {
...
void ActuatorData::setValueByUser(Actuator::ActuatorAttribute nDataId, QVariant value, bool bSendProxy)
{
        ...
        if(pId != DIRECTIVES_INVALID)
        {
            ...
            if(bSendProxy)
            {
                ...
                //case D_SET_DEVICE_ID:
                {
                    ...
                    //qDebug()<<"send" << value.toInt() << getValue(ACTUATOR_SWITCH).toInt();
                }
                    break;
                case D_SET_DEVICE_ID:
                    InnfosProxy::SendProxy(deviceId(),(quint32)getValue(Actuator::SN_ID).toUInt(),Actuator::D_SET_DEVICE_ID,quint8(value.toUInt()));
                    break;
                case D_CHECK_ERROR:
                    if(value.toInt() == Actuator::ERR_NONE)//clear error
                    {
                ...
                case D_CHART_OPEN:
                {
                    if(value.toInt() == Actuator::CHART_SWITCH_ON)
                    {
...
void ActuatorData::responseHeart(bool bSuccess)
{
    ...
    {
        m_nHeartFailCnt = 0;
        if(getValue(ONLINE_STATUS).toInt() == Actuator::Status_Offline)//这种情况只有断线重连才会判断成功，重连后m_nHeartFailCnt，但是电机数据在线状态还是断线
        {
...
QVariant MotorDataMgr::getMotorDataAttrValue(const quint8 nDeviceId, const Actuator::ActuatorAttribute attrId) const
{
    ActuatorData * pData = getMotorDataById(nDeviceId);
    if(pData)
...
qreal MotorDataMgr::getMotorDataAttrValueAsDouble(const quint8 nDeviceId, const Actuator::ActuatorAttribute attrId) const
{
    bool bOK = false;
...
qint32 MotorDataMgr::getMotorDataAttrValueAsInt(const quint8 nDeviceId, const Actuator::ActuatorAttribute attrId) const
{
    bool bOK = false;
...
void MotorDataMgr::setMotorDataAttrByUser(const quint8 nDeviceId, const Actuator::ActuatorAttribute attrId, QVariant value, bool bSend)
{
    ActuatorData * pData = getMotorDataById(nDeviceId);
    if(pData)
    {
        ...
        case ACTUATOR_SWITCH:
            if(/*pData->getValue(Actuator::ACTUATOR_SWITCH).toInt() != value.toInt()*/true)//if current value is equal to the target value,nothing will be done!
            {
...
void MotorDataMgr::setMotorDataAttrInBatch(const QList<quint8> idList, const Actuator::ActuatorAttribute attrId, QVariant value, bool bSend)
{
...
void MotorDataMgr::AddMotorsData(QMap<quint8, quint32> dataMap)
{
    ...
    while (it.hasNext())
    {
        ...
        ActuatorData * pData = new ActuatorData(it.key(),it.value(),this);
...
QMap<quint8, quint32> MotorDataMgr::getMotorsMap() const
{
    ...
    for(int i=0;i<m_allMotorDatas.size();++i)
    {
        ActuatorData * pData = m_allMotorDatas.at(i);
...
void MotorDataMgr::activeMotorModeInBatch(const QList<quint8> idList, const Actuator::ActuatorMode mode)
{
    for(int i=0;i<idList.size();++i)
    {
        setMotorDataAttrByUser(idList.at(i),MODE_ID,mode);
    }
}
...
void MotorDataMgr::regainData(const quint8 nDeviceId, Actuator::ActuatorAttribute attrId)
{
    ActuatorData * pData = getMotorDataById(nDeviceId);
    if(pData)
    {
...
{
    ActuatorData * pData = getMotorDataByNewId(nNewDeviceId);
    if(pData)
    {
...
void MotorDataMgr::requestSuccessfully(const quint8 nDeviceId, const quint8 nProxyId)
{
    ...
    {
        pData = getMotorDataByNewId(nDeviceId);
    }
    ...
    if(pData)
    {
        ...
        {
            qDebug() << "Switch";
            quint8 nRequest = pData->getUserRequestValue(ACTUATOR_SWITCH).toUInt();
            quint8 nBefore = pData->getValue(ACTUATOR_SWITCH).toUInt();
            if(nRequest==Actuator::ACTUATOR_SWITCH_ON && nBefore==Actuator::ACTUATOR_SWITCH_OFF)//user open motor
            {
                QTimer::singleShot(3000,[=]{pData->requestAllValue();});
            }
            else if(nRequest==Actuator::ACTUATOR_SWITCH_OFF && nBefore==Actuator::ACTUATOR_SWITCH_ON)//user close motor
            {
...
void MotorDataMgr::requestFailed(const quint8 nDeviceId, const quint8 nProxyId)
{
     ActuatorData * pData = getMotorDataById(nDeviceId);
     if(pData)
     {
         ...
         case D_SET_SWITCH_MOTORS:
         {
             ...
             {
                mediator->errorOccur(nDeviceId,Actuator::ERR_SHUTDOWN_SAVING,tr("Failed to save parameters when shutdown actuator %1!").arg(nDeviceId));
                requestSuccessfully(nDeviceId,nProxyId);//默认执行器已经关机
             }
         }
...
ActuatorData *MotorDataMgr::getMotorDataById(const quint8 nId) const
{
    for(int i=0;i<m_allMotorDatas.size();++i)
    {
        ActuatorData * pData = m_allMotorDatas.at(i);
...
ActuatorData *MotorDataMgr::getMotorDataByNewId(const quint8 nId) const
{
    for(int i=0;i<m_allMotorDatas.size();++i)
    {
        ActuatorData * pData = m_allMotorDatas.at(i);
...
ActuatorData *MotorDataMgr::getMotorDataByMac(const quint32 nMac) const
{
    for(int i=0;i<m_allMotorDatas.size();++i)
    {
        ActuatorData * pData = m_allMotorDatas.at(i);
